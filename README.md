...
Admin endpoints now require bearer token from `admin@local` login.
Example:

```bash
ADMIN=$(curl -sS -X POST http://127.0.0.1:8080/auth/login -H "content-type: application/json" -d '{"email":"admin@local","password":"admin"}' | python3 -c "import sys,json;print(json.load(sys.stdin)['access_token'])")
